...
void load_intrinsic_avx(sn_intrinsic *context, SIMDID id)
{
        ...
        context->mat_mk[F32].fma = is_fma ? GET_MK_PROC_STD(float, avx, , fma) :
                                            GET_MK_PROC_STD(float, avx, _fma, fma);
        context->mat_mk[F32].pivot = NULL;
        ...
        context->mat_mk[F64].transpose = NULL;
        context->mat_mk[F64].fma = is_fma ? GET_MK_PROC_STD(double, avx, , fma) :
                                            GET_MK_PROC_STD(double, avx, _fma, fma);
        context->mat_mk[F64].pivot = NULL;
